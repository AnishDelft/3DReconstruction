...
function [match1, match2] = findMatches(im1, im2, threshold, verbose)

    % Find features and make descriptor of image 1
    loc1                  = DoG(im1, 0.8, 0); %loc1 = [N,3]
    if (verbose == 1)
        fprintf('\n[DoG] Total points : %d', size(loc1,1));
    end
    [r1, c1, sigma1]      = harris(im1, loc1, verbose, 1);
    orient1               = zeros(size(sigma1));
    return;

    % Pay attention to the oder of parameters [c',r'] (equal to [x,y])
    [coord1, descriptor1] = sift(single(im1), 'frames', [c1'; r1'; sigma1'; orient1']);
...
% 
%             % Normalize the descriptors to unit L2 norm: [already done]
%             desc1 = ...
%             desc2 = ...
% 
%             % Compute the Euclidian distance of desc1 and desc2
%             dist = pdist([[1 1];[2 2]], 'euclidean')
% 
