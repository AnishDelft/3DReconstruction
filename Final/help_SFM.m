function [M,S,p] = help_SFM(X,Y,project_name) % X = [N_f, N_pts]; Y = [N_f, N_pts]
    ...
    save(strcat('Data/', project_name, 'SFM_M_', project_name), 'M');
    ...
    try
        ...
        [L, ~, ~, ~, output]     = lsqnonlin(@(L) help_residuals_template(L, project_name), L0, [], [],  opts1); % [3,3] 
        [C,p] = chol(L, 'lower');                   % [3,3]
        M     = M*C;                                % [202,3]
        S     = pinv(C)*S;                           % [3,215]
    catch e
       fprintf('\n  --> [Err-SFM] C : %d %d || p:%d || iters : %d', size(C),p, output.iterations);
